...
#define LAUNCHERS_AMOUNT            2           // Not a variable, should not be changed
...
static int score = 0;
static FireModes fireMode = SINGLE_MISSILE;
...
static void UpdateOutgoingSingleMissile();
...
{   
    ...
    for (int i = 0; i < BUILDINGS_AMOUNT; i++)  building[i].position = (Vector2){ (i + 2) * sparcing, screenHeight - BUILDING_SIZE/2 };
    launcher[1].position = (Vector2){ 9 * sparcing, screenHeight - LAUNCHER_SIZE/2 };
...
static void UpdateOutgoingSingleMissile()
{
    ...
    if (IsMouseButtonPressed(MOUSE_RIGHT_BUTTON)) launcherShooting = 2;
